...
def make_plot(cluster_name, core_count):
    results = pd.read_csv(
        os.path.join(common.results_dir, "concdedup.csv")
    )
    ...
    posix_time = (
        results[results["lock"] == "mutex"]
        .groupby("concurrent_num-threads")["run_time"]
        .mean()
    )
    results["speedup"] = (
        results["concurrent_num-threads"].map(posix_time) / results["run_time"]
    )
    ...
    common.save_fig(plt, f"concdedup-{cluster_name}")
