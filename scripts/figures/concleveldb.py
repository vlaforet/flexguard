...
def make_plot(cluster_name, bench_name, core_count, power_of_10=6):
    results = pd.read_csv(
        os.path.join(common.results_dir, "concleveldb.csv")
    )

    if core_count == 104:
        results = results[
            (results["concurrent_num-threads"] <= 256)
            & (results["concurrent_num-threads"] != 96)
        ]
    ...
    common.save_fig(plt, f"concleveldb-{bench_name}-{cluster_name}")
