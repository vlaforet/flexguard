...
THREADS="1 5 10"
TESTS="fillseq fillsync fillrandom overwrite readrandom readrandom readseq readreverse compact readrandom readseq readreverse fill100K crc32c snappycomp snappyuncomp zstdcomp zstduncomp"
ENTRIES=1000000
OUTPUT="results/leveldb.csv"
...
for n in $THREADS; do
    echo "threads=$n"
    res=$(ext/leveldb/build/db_bench --threads=$n --num=$ENTRIES --benchmarks="$(echo $TESTS | tr ' ' ',')")
