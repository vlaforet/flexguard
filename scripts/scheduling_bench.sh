...
./scheduling -n ${MAX_THREAD_COUNT} -s -1 -d ${DELAY} > tmp/hybrid.csv
...
./scheduling -n $MAX_THREAD_COUNT -s ${CORE_COUNT} -d ${DELAY} | tee tmp/hybrid_emulated.csv

echo Starting Hybrid Lock (No BPF) spin computation
./scheduling -n ${MAX_THREAD_COUNT} -s -1 -d ${DELAY} | tee tmp/hybrid_spin.csv

echo Starting Hybrid Lock (No BPF) blocking computation
./scheduling -n ${MAX_THREAD_COUNT} -s 0 -d ${DELAY} | tee tmp/hybrid_blocking.csv
...
./scheduling -n ${MAX_THREAD_COUNT} -d ${DELAY} | tee tmp/mcs.csv
...
./scheduling -n ${MAX_THREAD_COUNT} -d ${DELAY} | tee tmp/mutex.csv
...
./scheduling -n ${MAX_THREAD_COUNT} -d ${DELAY} | tee tmp/spin.csv